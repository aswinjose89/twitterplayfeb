...
(function () {
    var app = angular.module("histryUsersList", ['ngMaterial', 'ngMessages','Twitterplay.services', 'twitter.directives']);
    app.controller("histryUsersListController",usersHistoryController)
    .service("$histryUsersListSvc", usersHstryService);
    function usersHstryService(){    
    ...
    function usersHistoryController(remoteCall, $histryUsersListSvc, $mdToast, $log, $compile, $scope){
      var vm = this;  
      vm.searchInput= (localStorage.getItem("HstrysearchInput"))?JSON.parse(localStorage.getItem("HstrysearchInput")):{};
      function influentialUsersData(skip= 0){        
        let dataInput= {};
        dataInput.dateRange= vm.searchInput.tweet_created_date;
        dataInput.limit= 10;
        dataInput.skip= skip;
        remoteCall.get("/getInfluentialUsers", dataInput).then((result)=>{
          if(result.data.status === "success"){
              vm.usersProfileData= result.data.data;
          }
        });
      }
      vm.changeFriendship= function(friendship){
        bootbox.confirm("Excessive use of Follow/Unfollow options may result in your account getting blocked by Twitter. Do you want to continue?", function(confirmation) {
          if(confirmation){
            var user_id_arr=[];
            var flag = true;
            result= vm.usersProfileData.filter((x)=>x.selected);
            for(var i=0; i<result.length; i++){
              if($("#item_"+i).is(':checked')){
                if($('#'+result[i].screen_name).find('span').text() && $('#'+result[i].screen_name).find('span').text().toUpperCase()==="FOLLOWING" && friendship === 'create'){
                    Example.show("User <b>@"+result[i].screen_name+"</b> is already in your following list.");
